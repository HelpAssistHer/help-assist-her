...
describe('PregnancyCenters', () => {
	...
	/*
	 * Test the /GET /api/pregnancy-centers route -- authenticated 2
	 */
	describe('/GET /api/pregnancy-centers', () => {
		it('it should return an array with one pregnancy center', async () => {
			const pregnancyCenter = new PregnancyCenterModel({
				address: {
					line1:'586 Central Ave.\nAlbany, NY 12206',
					location:{
						'type':'Point',
			...
			const createdPregnancyCenter = await pregnancyCenter.save()
			...
			res.body.should.be.a('array')
			res.body.length.should.be.eql(1)
			res.body[0].name.should.equal(createdPregnancyCenter.name)
			res.body[0].should.have.property('primaryContactPerson')
		})
	})
	...
	describe('/POST /api/pregnancy-centers', () => {
		it('it should create a new pregnancy center and return the data', async () => {
			...
			const createdPC = await PregnancyCenterModel.findById(res.body._id)
			createdPC.should.be.a('object')
		})
	})
